...
try:
    ...
    print("❌  Run: pip install mysql-connector-python"); sys.exit(1)

app = Flask(__name__, static_folder=".")
app.secret_key = os.environ.get("SECRET_KEY", "finup_2026_secret")
CORS(app, origins="*")
...
def sync_storage(conn, user_id=None):
    try:
        ...
        if existing_files:
            placeholders = ",".join("%s" for _ in existing_files)
            files_tuple = tuple(existing_files)
            if user_id:
                cursor.execute(
                    f"DELETE FROM transactions WHERE user_id=%s AND source_file NOT IN ({placeholders})",
                    (user_id, *files_tuple)
                )
                cursor.execute(
                    f"DELETE FROM reports WHERE user_id=%s AND {report_file_col} NOT IN ({placeholders})",
                    (user_id, *files_tuple)
                )
            else:
                cursor.execute(f"DELETE FROM transactions WHERE source_file NOT IN ({placeholders})", files_tuple)
                cursor.execute(f"DELETE FROM reports WHERE {report_file_col} NOT IN ({placeholders})", files_tuple)
        else:
            if user_id:
...
def fetch_goal_state(conn, user_id):
    cursor = conn.cursor(dictionary=True)
    cursor.execute(
        "SELECT * FROM goals WHERE user_id=%s AND completed=0 ORDER BY created_at DESC",
        (user_id,)
    )
    goals = [_goal_to_dict(r) for r in cursor.fetchall()]
    cursor.execute(
        "SELECT * FROM achieved_goals WHERE user_id=%s ORDER BY achieved_at DESC",
        (user_id,)
    )
...
def init_db():
    ...
    # Index (ignore error if it already exists)
    try:
        cur.execute("CREATE UNIQUE INDEX idx_users_username ON users(username)")
...
def auth_signup():
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO users (id, username, display_name, password_hash, created_at) VALUES (%s,%s,%s,%s,%s)",
...
def _clean_desc(raw):
    ...
    _SKIP = {
        ...
        "MOB","SELFFT",
        "P2A","P2M","P2C","P2P","P2V","P2U",
        "UPI CO","COLLEC","PAYMEN","REMARK",
        "PHONE","UTIB","SBIN","HDFC","ICICI",
    }
    for p in s.split("/")[1:]:
        p = p.strip()
        if not p: continue
...
def reload_txns(conn, user_id):
    ...
    cursor = conn.cursor(dictionary=True)
    cursor.execute(
        "SELECT * FROM transactions WHERE user_id=%s ORDER BY date ASC",
        (user_id,)
    )
    rows = cursor.fetchall()
    cursor.close()
    # Normalize date fields from MySQL date objects to strings
    for r in rows:
...
def upload_statement():
    ...
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM transactions WHERE user_id=%s AND source_file=%s", (user_id, storage_name))
    cursor.execute("DELETE FROM reports WHERE user_id=%s AND storage_name=%s", (user_id, storage_name))
    conn.commit()
    for t in result.get("transactions", []):
        rid = str(uuid.uuid4())
        cursor.execute(
            "INSERT IGNORE INTO transactions (id, date, description, amount, type, category, mode, is_card, balance, confidence, source_file, user_id) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
    ...
    cursor.execute(
        "INSERT INTO reports (id, name, filed_at, total_income, total_debit, tx_count, report_excel_b64, summary_json, period, account_holder, user_id, storage_name) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
    ...
    cursor2.execute("SELECT * FROM reports WHERE user_id=%s ORDER BY filed_at DESC", (user_id,))
    filed = [dict(r) for r in cursor2.fetchall()]
    cursor2.close()
    cursor.close()
...
def download_report(rid):
    ...
    conn = get_db()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT name, report_excel_b64 FROM reports WHERE id=%s AND user_id=%s", (rid, user_id))
    row = cursor.fetchone()
...
def get_transactions():
    ...
    filtered = [t for t in DB["transactions"] if (str(t.get("date","")) or "") >= cutoff]
...
def bootstrap_state():
    ...
    try:
        ...
        if scoped_name:
            cursor.execute(
                "SELECT * FROM transactions WHERE user_id=%s AND source_file=%s ORDER BY date ASC",
                (user_id, scoped_name)
            )
        else:
        ...
        for r in rows:
            if hasattr(r.get("date"), "strftime"):
                r["date"] = r["date"].strftime("%Y-%m-%d")
        cursor.execute("SELECT * FROM reports WHERE user_id=%s ORDER BY filed_at DESC", (user_id,))
...
def add_goal():
    ...
    cursor = conn.cursor(dictionary=True)
    cursor.execute(
        "INSERT INTO goals (id, name, target, saved, color, created_at, deadline, completed, user_id) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)",
        (g["id"],g["name"],g["target"],g["saved"],g["color"],g["created_at"],g["deadline"],g["completed"], user_id)
    )
    if completed:
        cursor.execute("SELECT id FROM achieved_goals WHERE id=%s AND user_id=%s", (g["id"], user_id))
        if not cursor.fetchone():
            cursor.execute(
                "INSERT INTO achieved_goals (id, name, target, color, achieved_at, user_id) VALUES (%s,%s,%s,%s,%s,%s)",
...
def update_goal(gid):
    ...
    conn  = get_db()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT * FROM goals WHERE id=%s AND user_id=%s", (gid, user_id))
    row   = cursor.fetchone()
    just_completed = False
    if row:
        completed = 1 if saved >= row["target"] else 0
        just_completed = completed==1 and not bool(row["completed"])
        cursor.execute("UPDATE goals SET saved=%s, completed=%s WHERE id=%s AND user_id=%s", (saved,completed,gid,user_id))
        if just_completed:
            cursor.execute("SELECT id FROM achieved_goals WHERE id=%s AND user_id=%s", (gid, user_id))
            if not cursor.fetchone():
                cursor.execute(
                    "INSERT INTO achieved_goals (id, name, target, color, achieved_at, user_id) VALUES (%s,%s,%s,%s,%s,%s)",
    ...
    conn.commit()
    cursor.execute("SELECT * FROM goals WHERE id=%s AND user_id=%s", (gid, user_id))
    updated = _goal_to_dict(cursor.fetchone())
    cursor.close(); conn.close()
...
def delete_goal(gid):
    ...
    cursor.execute("DELETE FROM achieved_goals WHERE id=%s AND user_id=%s", (gid, user_id))
...
def delete_report(rid):
    ...
    conn = get_db()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT name, storage_name FROM reports WHERE id=%s AND user_id=%s", (rid, user_id))
    row  = cursor.fetchone()
    if row:
        ...
        try:
            ...
            pass
    cursor.execute("DELETE FROM reports WHERE id=%s AND user_id=%s", (rid, user_id))
    conn.commit()
    reload_txns(conn, user_id)
    cursor.execute("SELECT * FROM reports WHERE user_id=%s ORDER BY filed_at DESC", (user_id,))
    reports = [dict(r) for r in cursor.fetchall()]
...
def db_status():
    try:
        ...
        cursor.fetchone()
        cursor.close(); conn.close()
        return jsonify({"connected": True, "message": "MySQL connected ✅", "engine": "mysql"})
    except mysql.connector.Error as e:
        return jsonify({"connected": False, "message": str(e), "engine": "mysql"}), 500
